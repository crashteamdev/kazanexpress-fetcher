...
class PositionProductFetchMasterJob : Job {
    ...
    override fun execute(jobContext: JobExecutionContext) {
        ...
        for (categoryId in categoryIds) {
            val jobIdentity = "${categoryId}-position-product-fetch-job"
            val jobDetail =
                JobBuilder.newJob(PositionProductFetchJob::class.java).withIdentity(jobIdentity).build()
